...
def scheduler_get_create():
    ...
    return render_template(
        "schedulers/scheduler_create.html", appliances=appliances
    )
...
def scheduler_get_update(id):
    if not id or id != 0:
        ...
        if scheduler:
            return render_template(
                "schedulers/scheduler_update.html",
                scheduler=scheduler,
...
def scheduler_get_view(id):
    if not id or id != 0:
        ...
        if scheduler:
            return render_template(
                "schedulers/scheduler_view.html", scheduler=scheduler
            )
...
def scheduler_post_update():
    ...
    if scheduler:
        ...
        scheduler.appliances = Appliance.query.filter(
            Appliance.id.in_(appliances)
        ).all()
        scheduler.appliance_state = appliance_state

        if appliance_state != old_appliance_state:
