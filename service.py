...
def add_scheduler(scheduler) -> None:
    ...
    if not cron_scheduler.get_job(scheduler.name):
        ...
        dict_args = {
            cron_time_list[i]: val
            for i, val in enumerate(scheduler.cron.split())
        }
        cron_scheduler.add_job(
            func=set_gpio_appliances_from_scheduler,
            id=scheduler.name,
            trigger="cron",
            args=[appliances, scheduler.__dict__["appliance_state"]],
            **dict_args,
        )
